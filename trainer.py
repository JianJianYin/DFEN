...
from test_isic import inference_isic
def trainer_synapse(args, model, snapshot_path):
    ...
    for epoch_num in iterator:
        ...
        if epoch_num >= (max_epoch-40) : #保存最后30轮的结果
            ...
            args.volume_path = "/home/JianjianYin/transdeeplab/data/Synapse/test_vol_h5"
            #performance = inference(args, model, None)
            #if performance > best_performance:
            filename = f'{args.config_file}_epoch_{epoch_num}.pth'
            save_mode_path = os.path.join("/home/JianjianYin/transISNet/Synapse/Synapse_5_5", filename)
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            if iter_num % 20 == 0:
                ...
                writer.add_image('train/GroundTruth', labs, iter_num)
        model.eval()
        cur_performance = inference_isic(args, model)
        if cur_performance > best_performance:
            best_performance = cur_performance
            torch.save(model.state_dict(), "/home/y212202015/KBS/TransISNet/experiment/0_0_6_24/best.pth")
        model.train()
        logging.info("当前最好的DSC为%s"%(best_performance))

        # save_interval = 10
        # if epoch_num >= int(max_epoch-70) : #保存最后30轮的结果
        #     filename = f'{args.config_file}_epoch_{epoch_num}.pth'
        #     save_mode_path = os.path.join("/home/y212202015/KBS/TransISNet/isic2018/0_0_1", filename)
        #     torch.save(model.state_dict(), save_mode_path)
        #     logging.info("save model to {}".format(save_mode_path))
...
def trainer_busi(args, model, snapshot_path):
    ...
    for epoch in range(max_train_epoch):
        ...
        if val_log['dice'] > best_dice:
            torch.save(model.state_dict(), '/home/JianjianYin/transISNet/BUSI/model_1.pth')
            best_dice = val_log['dice']
            print("=> saved best model")
        print("最好的结果为:%s"%(best_dice))
