...
class SwinASPP(nn.Module):
    def __init__(self, input_size, input_dim, out_dim, cross_attn,
        ...
        for ws in self.possible_window_sizes:
            layer = BasicLayer(dim=int(input_dim), # 384
    ...
    def forward(self, x):
        ...
        features = self.proj(features) # 输出的维度变成了32
...
def build_aspp(input_size, input_dim, out_dim, config):
    ...
    if config.aspp_name == 'swin':
        return SwinASPP(
            ...
            cross_attn=config.cross_attn, # CBAM
            num_heads=config.num_heads, # 3头
            ...
            aspp_activation=config.aspp_activation, # relu
            start_window_size=config.start_window_size,
            aspp_dropout=config.aspp_dropout, # 0.1
            downsample=config.downsample, #None
