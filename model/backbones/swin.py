...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        if self.shift_size > 0: # 是否进行Shifted Window Multi-head  self attention 
...
class SwinEncoder(nn.Module):
    def __init__(self, img_size=224, patch_size=4, in_chans=3,
                 ...
                 norm_layer=nn.LayerNorm, high_level_norm=False, low_level_norm=False, middle_level_norm = False,ape=False, patch_norm=True,
        ...
        self.low_level_after_block = low_level_after_block  # True
        self.high_level_after_block = high_level_after_block # True

        self.patch_embed = PatchEmbed(
            img_size=img_size, patch_size=patch_size, in_chans=in_chans, embed_dim=embed_dim,
            norm_layer=norm_layer if self.patch_norm else None) # 将图片进行卷积之后打成块
        ...
        for i_layer in range(self.num_layers): # 构建 Swin的块
            layer = BasicLayer(dim=int(embed_dim * 2 ** i_layer),# 96,192,384
                               input_resolution=(patches_resolution[0] // (2 ** i_layer),
                                                 patches_resolution[1] // (2 ** i_layer)),
                               depth=depths[i_layer], # 对应的块
                               num_heads=num_heads[i_layer], # [3,6,12]
                               ...
                               norm_layer=norm_layer,
                               downsample=PatchMerging if (i_layer < self.num_layers - 1) else None, #除最后一层外，其他都使用patchmerging
                               use_checkpoint=use_checkpoint)
            self.layers.append(layer)
            
        # storing sizes and dimensions of the outputs
        self.high_level_size = img_size // (4 * 2**high_level_idx) # 14
        self.high_level_dim = int(embed_dim * 2 ** high_level_idx) # 384
        self.low_level_dim = int(embed_dim * 2 ** low_level_idx) # 96
        ...
        self.middle_level_norm = norm_layer(192) if middle_level_norm else None
    ...
    def forward(self, x):
        ...
        x = self.patch_embed(x) # 划分成块，为56*56个块
        if self.ape:
            x = x + self.absolute_pos_embed
        x = self.pos_drop(x)

        low_level = high_level = None
        if self.low_level_idx == 0 and not self.low_level_after_block: # False
            low_level = x
        ...
        for layer in self.layers: # 3 
            x, down = layer(x if down is None else down) # 最后一层是没有down的
            if depth == 1 :
                middle_level = x 
            if depth == self.low_level_idx and self.low_level_after_block: # 使用block之后的特征
                low_level = x
        ...
        if self.middle_level_norm is not None:
            middle_level = self.middle_level_norm(middle_level)
            
        low_size = int(math.sqrt(low_level.size(1))) # 经过平方之后得到的结果
        middle_size = int(math.sqrt(middle_level.size(1)))
        high_size = int(math.sqrt(high_level.size(1)))

        low_level = low_level.view(-1, low_size, low_size, low_level.shape[-1])
        middle_level = middle_level.view(-1,middle_size,middle_size,middle_level.shape[-1])
